...
float4 PS( float2 ScreenSpace : VPOS ) : COLOR0
{
	...
	// dchapman: Transform the light orientation vector into view-space and calculate the cos angle between it and the light vector  - 05/01/2012
	float cosAngle = max(dot( -toLight.xy, mul(LightOrientation, View).xy ), 0);

	// dchapman: Modify the distance attenuation factor according to the falloff angles of the spotlight cone - 05/01/2012
	if ( cosAngle < OuterCosAngle )
		attenuation *= 0.0f;
	else if ( cosAngle < InnerCosAngle )
		attenuation *= (cosAngle - OuterCosAngle) / (InnerCosAngle - OuterCosAngle);
